...
var pre_fireSword = preload("res://scenes/fireSword.tscn")#this function preloads the arrow scene will be used in arrow function
...
func dataAttack():
	if action:
		...
		if animation == "shield":
			...
			yield(get_tree().create_timer(0.82), "timeout")
			fireSword()
...
func fireSword():
	...
	var fireSword = pre_fireSword.instance()
	get_parent().add_child(fireSword)
	#$arrow.play()
